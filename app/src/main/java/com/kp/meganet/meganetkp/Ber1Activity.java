...
public class Ber1Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Q3.setFilters(new InputFilter[]{new InputFilterMinMax("-9.99","9.99")});
        Q03.setFilters(new InputFilter[]{new InputFilterMinMax("-9.99","9.99")});
        Q2.setFilters(new InputFilter[]{new InputFilterMinMax("-9.99","9.99")});
        Q1.setFilters(new InputFilter[]{new InputFilterMinMax("-9.99","9.99")});
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (!hasFocus) {
                    if(res<1 && acc*res == Double.valueOf(String.valueOf(accumulation.getText())))
                    {
                        ...
                        acc = Double.valueOf(accumulation.getText().toString().replace(".",""));
                        pos = acc + neg;

                        if (accUnitSpinner.getSelectedItem().toString().equals("M3")) {
                            res = Double.valueOf(resolutionSpinner.getSelectedItem().toString());
                            if (res < 1) {
                                accumulation.setText(String.valueOf(fmt(acc * res)));
                                negative.setText(String.valueOf(fmt(neg * res)));
                                positive.setText(String.valueOf(fmt(pos * res)));
                            }
                            else {
                                accumulation.setText(String.valueOf(fmt(acc)));
                                negative.setText(String.valueOf(fmt(neg)));
                                positive.setText(String.valueOf(fmt(pos)));
                            }
                        }
                    }
                }
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (!hasFocus) {
                    if(res<1 && neg*res == Double.valueOf(String.valueOf(negative.getText())))
                    {
                        ...
                        neg = Double.valueOf(negative.getText().toString().replace(".",""));
                        pos = acc + neg;
        ...
        acc = Double.valueOf(accumulation.getText().toString().replace(".",""));
        neg = Double.valueOf(negative.getText().toString().replace(".",""));
        pos = Double.valueOf(positive.getText().toString().replace(".",""));
        ...
        for(i=0;i<dataArr.length;i++)
        {
            ...
            switch (line[0])
            {
                ...
                case "Meter Type":
                    ...
                    sizeSpinnerArrayAdapter.setDropDownViewResource(R.layout.spinner_item);
                    sizeSpinner.setAdapter(sizeSpinnerArrayAdapter);
                    //sizeSpinnerArrayAdapter.notifyDataSetChanged();
                    break;
                case "Size":
    ...
    public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
        try {
            ...
            if (isInRange(min, max, input)) {
                int index = String.valueOf(dest).indexOf(".");
                if(index >= 0 && index < dest.length()-2) { }
                else return null;
            }
        } catch (NumberFormatException nfe) {
            if(source.toString().equals("-")||source.toString().equals(".")) {
                if(dest.toString().equals(".") && source.toString().equals("-") ||
                        source.toString().equals(".") && dest.toString().equals("") && dend == 0)
                    return null;
            }
        }
        return "";
